...
**Администратор попадёт в `/admin` только со вторым фактором.** Роль Admin обязана
подключить аутентификатор (`Профиль → Безопасность → Двухфакторная аутентификация`);
до этого админка переадресует на страницу настройки, а `/api/admin/*` отвечает 403.
Модератору второй фактор не нужен.
