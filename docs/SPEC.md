...
> Реализация идёт строго по плану работ (§17), одна фаза = одна сессия Claude Code = один Pull Request.
...
**Правило при нехватке времени:** резать можно только из раздела «Приоритет B» (§16). Всё из «Приоритета A» обязательно.
...
|---|---|---|---|
| 1 | Архитектура | **5** — Clean Architecture, отдельные проекты | 4 проекта + 2 тестовых, правило зависимостей, автотесты архитектуры (§4) |
| 2 | Дизайн и ТЗ | **5** — адаптив, макеты, SPA-подход | Токены из макета, mobile-first, partial-навигация + History API (§8.1) |
| 3 | AJAX | **5** — 5+ запросов | 10 сценариев (§9.1) |
| 4 | Админ-панель | **5** — 6+ страниц | 13 страниц (§8.4) |
| 5 | Авторизация и роли | **5** — Identity, 3 роли, политики | Admin / Moderator / User + политики + динамические фича-политики (§7) |
| 6 | Страницы | **5** — 6+, ≥3 динамических | 17 публичных, 13 динамических (§8.3) |
| 7 | Фильтрация | **5** — состояние в URL, переживает F5 | Единый механизм на 3 списках (§9.2) |
| 8 | Валидация | **5** — клиент + сервер | DataAnnotations + Unobtrusive + Remote / FluentValidation (§14) |
| 9 | Пагинация | **5** — AJAX с сохранением состояния | Единый tag helper, состояние в URL (§9.3) |
| 10 | Структура БД | **5** — 6+ таблиц, нормализовано | 30 таблиц, 3НФ (§5) |
| 11 | SignalR | **5** — полноценный чат/уведомления | Live-счёт матча + чат матча + персональные уведомления (§10) |
| 12 | Локализация | **5** — 2+ языка, всё локализовано | ru/en: UI, валидация, письма, ошибки, enum'ы + переводы контента в БД (§8.5) |
| 13 | MongoDB | **5** — реальные задачи | Лента событий матча, чат, аудит, логи + альтернативное хранилище новостей (§11) |
| 14 | Логирование | **5** — действия пользователя | Serilog + `AuditBehavior` + просмотр в админке (§12) |
| 15 | Маршруты/ошибки | **5** — свои страницы ошибок | 400/401/403/404/429/500, локализованные (§13.2) |
| 16 | Middleware | **5** — минимум один полезный | 4 собственных (§13.1) |
...
## 3. Предметная область и главный сценарий
...
### 4.4. Точки расширения (описать в `docs/extension-points.md`)
...
### 4.5. SOLID — примеры для защиты (в `architecture.md`)
...
## 5. Модель данных (30 таблиц, PostgreSQL)
...
## 6. Подписки и разграничение доступа
...
### 6.6. Мнимая оплата с подтверждением по коду
...
## 7. Аутентификация и безопасность аккаунта
...
### 7.2. Требования безопасности (прямое требование задания)
...
### 8.3. Публичные страницы (17, динамических 13)
...
| 14 | `/checkout` | Оформление: адрес доставки, список выбранных товаров, платёжные реквизиты, подтверждение кодом из письма (§6.6) | ✔ |
...
### 8.4. Админ-панель — Area `Admin` (13 страниц, нужно ≥6)
...
## 9. AJAX, фильтрация, пагинация

### 9.1. AJAX-сценарии (нужно 5+, делаем 10)
...
## 10. SignalR — один хаб `LiveHub` (`/hubs/live`)
...
* Обязательно логируются события из §7.2.
...
## 13. Middleware и ошибки

### 13.1. Собственные middleware (4)
...
## 15. Производительность и деплой
...
## 16. Приоритеты при нехватке времени
...
**Фаза 0 — Фундамент.** Solution, 4 + 2 проекта, `Directory.Packages.props`, `.editorconfig`, `.gitignore`, `global.json`, README, `docker-compose` (postgres + mongo + mailhog), CI, `CLAUDE.md`, архитектурные тесты (пока красные — как спецификация правил), ADR-0001.
...
**Фаза 8 — AJAX, фильтры, пагинация.** `Areas/Api`, все сценарии §9.1, `http.js`, `pushState`/`popstate`, скелетоны, тосты. Проверка: F5 сохраняет фильтр, «Назад» работает, ссылка на страницу 3 открывается корректно.
...
## 18. Definition of Done — чек-лист для защиты
